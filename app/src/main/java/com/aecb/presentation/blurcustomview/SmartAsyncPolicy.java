        ...
        if (isRenderScript) {
            synchronized (rsStatistics) {
                if (rsStatistics.sampleCount() <= 0) {
                    if (isDebug) {
                        Timber.d(TAG, String.format(Locale.US,
    ...
    public void putSampleData(boolean isRenderScript, long computation, long timeInNanos) {
        ...
        if (isRenderScript) {
            ...
            synchronized (rsStatistics) {
                ...
                if (isDebug) {
                    Timber.d(TAG, String.format(Locale.US, "Statistics(RS): timerPerComp-> %.3f ns, sampleCount-> %d",
            ...
            synchronized (nonRsStatistics) {
                ...
                if (isDebug) {
                    Timber.d(TAG, String.format(Locale.US, "Statistics(Non-RS): timerPerComp-> %.3f ns, sampleCount-> %d",
