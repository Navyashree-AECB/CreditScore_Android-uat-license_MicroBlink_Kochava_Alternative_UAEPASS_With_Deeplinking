...
class RenderScriptBlur extends BaseBlurEngine {
    ...
    public synchronized static boolean isAvailable(@NonNull Context context) {
        if (!isAvailabilityChecked) {
            ...
            try {
                ...
                Timber.w(TAG, "Renderscript is not available on this device.");
                available = false;
            } finally {
                if (rs != null) {
                    rs.destroy();
    ...
    private Bitmap blur(Bitmap inBitmap, Bitmap outBitmap) {
        ...
        if (start > 0) {
            ...
            if (blurConfig.debug()) {
                Timber.d(TAG, String.format(Locale.US, "RenderScriptBlur took %d ms.", duration / 1000000L));
            }
        }
    ...
    class BlurAsyncTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if (blurConfig.debug()) {
                Timber.d(TAG, "Running in background...");
            }
            return blur(inBitmap, outBitmap);
        }
