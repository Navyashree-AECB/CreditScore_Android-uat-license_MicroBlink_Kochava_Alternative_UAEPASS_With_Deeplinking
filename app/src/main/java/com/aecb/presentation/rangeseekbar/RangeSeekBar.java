...
import static com.aecb.presentation.rangeseekbar.SeekBar.INDICATOR_ALWAYS_HIDE;
import static com.aecb.presentation.rangeseekbar.SeekBar.INDICATOR_ALWAYS_SHOW;
...
public class RangeSeekBar extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            progressHeight = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_progress_height, SeekbarUtils.dp2px(getContext(), 2));
            ...
            tickMarkTextMargin = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_margin, SeekbarUtils.dp2px(getContext(), 7));
            tickMarkTextSize = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_size, SeekbarUtils.dp2px(getContext(), 12));
    ...
    protected int getTickMarkRawHeight() {
        if (tickMarkTextArray != null && tickMarkTextArray.length > 0) {
            return tickMarkTextMargin + SeekbarUtils.measureText(String.valueOf(tickMarkTextArray[0]), tickMarkTextSize).height() + 3;
        }
        return 0;
    }
        ...
        if (tickMarkTextArray != null) {
            ...
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    ...
                    float num = SeekbarUtils.parseFloat(text2Draw);
                    SeekBarState[] states = getRangeSeekBarState();
                    if (SeekbarUtils.compareFloat(num, states[0].value) != -1 && SeekbarUtils.compareFloat(num, states[1].value) != 1 && (seekBarMode == SEEKBAR_MODE_RANGE)) {
    ...
    public void setStepsDrawable(List<Integer> stepsDrawableIds) {
        ...
        for (int i = 0; i < stepsDrawableIds.size(); i++) {
            stepsBitmaps.add(SeekbarUtils.drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableIds.get(i)));
        }
